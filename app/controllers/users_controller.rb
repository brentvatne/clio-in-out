class UsersController < ApplicationController
  ...
  before_action :only_myself, only: [:edit, :update]
  ...
  def status
    ...
    respond_to do |format|
      format.json {
        render json: @user.to_json(only: [:id, :status], methods: [:full_name])
      }
    end
  end
  ...
  def user_params
    params.require(:user).permit(:status, :email, :first_name, :last_name, :web_site)
  end
